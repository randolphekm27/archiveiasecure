...
export default function LoginPage() {
  const [mode, setMode] = useState<'login' | 'create' | 'forgot-password'>('login');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [success, setSuccess] = useState('');
  const [createdOrg, setCreatedOrg] = useState<{ code: string; username: string } | null>(null);
  const { signIn, createOrganization, resetPassword } = useAuth();
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      await resetPassword(resetForm.email, resetForm.orgCode);
      setSuccess('Un email de reinitialisation a ete envoye a votre adresse si elle est associee a cette organisation.');
    } catch (err) {
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          {mode === 'login' ? (
            ...
            <form onSubmit={handleResetPassword} className="space-y-4">
              <div className="text-sm text-slate-600 mb-4 bg-slate-50 p-4 border border-slate-200 rounded-lg">
                Pour réinitialiser votre mot de passe, nous devons vérifier votre identité ainsi que votre appartenance à l'organisation.
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    type="text"
                    required
                    value={resetForm.orgCode}
                    onChange={(e) => setResetForm({ ...resetForm, orgCode: e.target.value.trim().toUpperCase() })}
                    placeholder="Ex: MEMP_2024"
                    className="w-full pl-11 pr-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition"
                  />
                </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) => setResetForm({ ...resetForm, email: e.target.value.trim() })}
              ...
              <button
                ...
                className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-lg font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-blue-600/30"
              >
                {loading ? 'Envoi...' : 'Reinitialiser mon mot de passe'}
              </button>

              <button
                type="button"
                onClick={() => { setMode('login'); setError(''); setSuccess(''); }}
                className="w-full text-slate-600 hover:text-slate-900 py-2 font-medium transition-colors"
              >
